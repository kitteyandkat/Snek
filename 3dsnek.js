...
function distance(obj1, obj2) {
    ...
    let deltaY = (obj1.position.y - obj2.position.y) ** 2
...
//update the position of the camera
  ...
  updateTranslation(timeElapsedS) {
    ...
    if (window.camera.position.x < -45 || window.camera.position.x > 45){
...
//lighting to simulate the sun casting shadows on the objects
  ...
  step(timeElapsed) {
    ...
    //if distance between the camera and the treat is < 1, move the treat toa random location
    if(distance(window.camera, window.treat) < 1){
        ...
        let randomPosition = Math.random() * (treatMax - treatMin) + treatMin
    ...
    //copy paste collision code from 3d snek
